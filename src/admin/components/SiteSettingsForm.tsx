...
export function SiteSettingsForm({ section }: { section: 'content' | 'configuration' }) {
  ...
  const [previousFinalEditorialImagePath, setPreviousFinalEditorialImagePath] = useState('')
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const values = section === 'content' ? {
        heroTitle: settings.heroTitle, heroDescription: settings.heroDescription, heroImageUrl: settings.heroImageUrl, heroImagePath: settings.heroImagePath, heroImageZoom: settings.heroImageZoom, heroImagePositionX: settings.heroImagePositionX, heroImagePositionY: settings.heroImagePositionY, homeBeforeAfterTitle: settings.homeBeforeAfterTitle, homeBeforeAfterText: settings.homeBeforeAfterText, homeBeforeImageUrl: settings.homeBeforeImageUrl, homeBeforeImagePath: settings.homeBeforeImagePath, homeBeforeImageZoom: settings.homeBeforeImageZoom, homeBeforeImagePositionX: settings.homeBeforeImagePositionX, homeBeforeImagePositionY: settings.homeBeforeImagePositionY, homeAfterImageUrl: settings.homeAfterImageUrl, homeAfterImagePath: settings.homeAfterImagePath, homeAfterImageZoom: settings.homeAfterImageZoom, homeAfterImagePositionX: settings.homeAfterImagePositionX, homeAfterImagePositionY: settings.homeAfterImagePositionY, finalEditorialImageUrl: settings.finalEditorialImageUrl, finalEditorialImagePath: settings.finalEditorialImagePath, finalEditorialImageZoom: settings.finalEditorialImageZoom, finalEditorialImagePositionX: settings.finalEditorialImagePositionX, finalEditorialImagePositionY: settings.finalEditorialImagePositionY, aboutTitle: settings.aboutTitle, aboutText: settings.aboutText, ctaTitle: settings.ctaTitle, ctaDescription: settings.ctaDescription, formDisclaimer: settings.formDisclaimer, specialties: settings.specialties, homeVisualBlocks: settings.homeVisualBlocks, locationMapsUrl: settings.locationMapsUrl, locationEmbedUrl: settings.locationEmbedUrl, locationAddress: settings.locationAddress,
      } : {
      ...
      if (previousFinalEditorialImagePath && previousFinalEditorialImagePath !== settings.finalEditorialImagePath) await removeSiteImage(previousFinalEditorialImagePath)
      await Promise.all(Array.from(new Set(retiredHomeImagePaths)).filter(Boolean).map(removeSiteImage))
      setPreviousHeroImagePath(''); setPreviousFinalEditorialImagePath(''); setRetiredHomeImagePaths([]); setDirty(false); setMessage('Cambios guardados correctamente.')
  ...
  return (
    <form className="admin-panel admin-form admin-settings-form" onSubmit={submit}>
      {section === 'content' ? <>
        ...
        <div className="admin-form-section admin-form__wide"><p className="eyebrow">Imagen editorial final</p><h2>Fotografía antes de la llamada final</h2><p>Esta imagen aparece a todo lo ancho cerca del cierre de la Home. Si la quitás, el bloque se oculta sin mostrar placeholders.</p></div>
        <div className="admin-form__wide">
          <ImageUploadField folder="home" label="Imagen final" imageUrl={settings.finalEditorialImageUrl} imagePosition={{ zoom: settings.finalEditorialImageZoom, positionX: settings.finalEditorialImagePositionX, positionY: settings.finalEditorialImagePositionY }} onUploaded={(result) => { if (!previousFinalEditorialImagePath) setPreviousFinalEditorialImagePath(settings.finalEditorialImagePath); setDirty(true); setSettings((current) => current ? { ...current, finalEditorialImageUrl: result.publicUrl, finalEditorialImagePath: result.path, finalEditorialImageZoom: 1, finalEditorialImagePositionX: 50, finalEditorialImagePositionY: 50 } : current) }} />
          {settings.finalEditorialImageUrl && <div className="admin-inline-actions"><button className="admin-button admin-button--secondary" type="button" onClick={() => { if (!previousFinalEditorialImagePath) setPreviousFinalEditorialImagePath(settings.finalEditorialImagePath); setDirty(true); setSettings((current) => current ? { ...current, finalEditorialImageUrl: '', finalEditorialImagePath: '', finalEditorialImageZoom: DEFAULT_IMAGE_POSITION.zoom, finalEditorialImagePositionX: DEFAULT_IMAGE_POSITION.positionX, finalEditorialImagePositionY: DEFAULT_IMAGE_POSITION.positionY } : current) }}>Quitar imagen final</button></div>}
          <ImagePositionEditor usage="footer" imageUrl={settings.finalEditorialImageUrl} imageAlt="Vista previa de la imagen final" value={{ zoom: settings.finalEditorialImageZoom, positionX: settings.finalEditorialImagePositionX, positionY: settings.finalEditorialImagePositionY }} onSave={(position) => { update('finalEditorialImageZoom', position.zoom); update('finalEditorialImagePositionX', position.positionX); update('finalEditorialImagePositionY', position.positionY) }} />
        </div>
